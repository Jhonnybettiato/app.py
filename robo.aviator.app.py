...
st.set_page_config(page_title="Aviator Elite PY v6.3", page_icon="🦅", layout="wide")
...
st.title("🦅 Aviator Elite PY v6.3")
...
# --- SIDEBAR (Configuración y Edición de Horarios) ---
with st.sidebar:
    ...
    st.markdown("---")
    st.subheader("✍️ Editar Horarios")
    st.session_state.hora_10x = st.text_input("Última Rosa (10x):", value=st.session_state.hora_10x)
    st.session_state.hora_100x = st.text_input("Último Gigante (100x):", value=st.session_state.hora_100x)
...
def get_minutos(hora_str):
    ...
    try:
        ...
        h_r = pytz.timezone('America/Asuncion').localize(h_r)
        diff = ahora - h_r
...
with m1:
    st.markdown(f'<div class="metric-card" style="border-color:#ffffff;"><p style="margin:0;color:#bdc3c7;font-size:0.8rem;">SALDO</p><h2 style="margin:0;">{int(st.session_state.saldo_dinamico):,}</h2></div>', unsafe_allow_html=True)
with m2:
    st.markdown(f'<div class="metric-card" style="border-color:#00ff41;"><p style="margin:0;color:#bdc3c7;font-size:0.8rem;">GANANCIA</p><h2 style="margin:0;color:#00ff41;">+{int(max(0, ganancia_actual)):,}</h2></div>', unsafe_allow_html=True)
with m3:
    st.markdown(f'<div class="metric-card" style="border-color:#ff3131;"><p style="margin:0;color:#bdc3c7;font-size:0.8rem;">PÉRDIDA</p><h2 style="margin:0;color:#ff3131;">-{int(abs(min(0, ganancia_actual))):,}</h2></div>', unsafe_allow_html=True)
