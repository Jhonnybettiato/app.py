...
def obtener_semaforo():
    ...
    if "Hueco" in est:
        ...
        return ("🟢 ESPEJO ACTIVO", "#8e44ad") if rosas >= 2 else ("🔴 BUSCANDO PAR", "#c0392b")
    else:
        target = 1.5 if "1.50" in est else 2.0
        return ("🟢 APUESTE (RACHA)", "#27ae60") if hist[-1] >= target else ("🔴 NO ENTRAR", "#c0392b")
...
# FILA 4: ENTRADAS (DISEÑO v9.2 REESTABLECIDO)
st.markdown("<br>", unsafe_allow_html=True)
with st.form("panel_registro", clear_on_submit=True):
    col_in, col_ap, col_ck, col_undo = st.columns([2, 1, 1, 1])
    with col_in: valor_vuelo = st.text_input("VUELO:", placeholder="Pegue valor aquí")
