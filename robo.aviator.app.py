...
def obtener_semaforo():
    ...
    # 2. Lógica CAZADOR (Basado en racha de proximidad)
    elif "Cazador" in est:
        # Si hubo una rosa hace poco (entre 2 y 10 rondas), es zona de caza
        if 2 <= sin_rosa <= 10: return "🟢 ZONA DE CAZA", "#27ae60"
        return "🔴 ESPERAR SEÑAL", "#c0392b"
        
    # 3. Lógica ESPEJO (Si la última fue rosa, se busca la repetición)
    elif "Espejo" in est:
        if hist[-1] >= 10: return "🟢 POSIBLE ESPEJO", "#8e44ad"
        return "🔴 ESPERANDO ROSA", "#c0392b"
        
    # 4. Lógica CONSERVADORA (Basada en azules/rosas)
