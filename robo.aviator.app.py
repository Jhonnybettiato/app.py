...
st.set_page_config(page_title="Aviator Elite Robot v9.6.5", page_icon="🦅", layout="wide")
...
if 'capital_inicial' not in st.session_state: st.session_state.capital_inicial = 50000.0
if 'primer_arranque' not in st.session_state: 
    st.session_state.saldo_dinamico = 50000.0
    st.session_state.primer_arranque = False

# --- BARRA LATERAL (CONFIGURACIÓN RESTAURADA) ---
with st.sidebar:
    st.markdown("### ⚙️ AJUSTES PRO")
    nuevo_cap = st.number_input("Capital Inicial (Gs.)", value=int(st.session_state.capital_inicial), step=5000)
    if nuevo_cap != st.session_state.capital_inicial:
        diff = nuevo_cap - st.session_state.capital_inicial
        st.session_state.saldo_dinamico += diff
        st.session_state.capital_inicial = float(nuevo_cap)
    
    st.divider()
    st.markdown("### 🕒 EDITAR HORARIOS")
    edit_10x = st.text_input("Última 10x (HH:MM)", value=st.session_state.h_10x)
    edit_100x = st.text_input("Última 100x (HH:MM)", value=st.session_state.h_100x)
    
    if st.button("✅ APLICAR CAMBIOS"):
        st.session_state.h_10x = edit_10x
        st.session_state.h_100x = edit_100x
        st.rerun()

    st.divider()
    if st.button("🔄 REINICIAR ROBOT"):
...
# --- INTERFAZ GRAFICA ---
st.markdown('<div class="main-header">🦅 AVIATOR ELITE ROBOT v9.6.5</div>', unsafe_allow_html=True)
...
with cols[1]:
    res_acum = st.session_state.saldo_dinamico - st.session_state.capital_inicial
    cl = "win-box" if res_acum >= 0 else "loss-box"
    st.markdown(f'<div class="{cl}"><p class="label-elite">RESULTADO ACUM.</p><h2 class="valor-elite">{int(res_acum):,}</h2></div>', unsafe_allow_html=True)
with cols[2]:
...
with r1:
    st.text_input("VALOR DEL VUELO", value="", placeholder="Número + Enter", 
...
if st.button("🔙 DESHACER ÚLTIMA REGISTRO", use_container_width=True):
    if st.session_state.historial:
