...
st.set_page_config(page_title="Aviator Elite PY v3.8", page_icon="🦅", layout="wide")

# --- DISEÑO CSS DINÁMICO (COLORES FUERTES) ---
st.markdown("""
    <style>
    .main { background-color: #0e1117; color: #ffffff; }
    
    /* Estilo base para las métricas */
    ...
    [data-testid="stMetricLabel"] { color: #ced4da !important; font-weight: bold !important; text-transform: uppercase; }
...
def motor_semaforo(h, modo):
    if len(h) < 3: return "🟡 AMARILLO: ANALIZANDO", "#f1c40f", "black"
    ...
    if modo == "Conservadora (1.50x)":
        ...
        if sum(h[-3:])/3 >= 1.7: return "🟢 VERDE: HACER APUESTA", "#00ff41", "black"
        return "🟡 AMARILLO: ESPERAR", "#f1c40f", "black"
    else:
        if vuelos_desde_rosa >= 18: return "🟢 VERDE: SEÑAL ROSA", "#e91e63", "white"
        if vuelos_desde_rosa >= 12: return "🟡 AMARILLO: CICLO CERCA", "#f1c40f", "black"
...
# --- CÁLCULOS ---
div_apuesta = 5 if st.session_state.modo_juego == "Conservadora (1.50x)" else 25
sugerida = (saldo_in * (obj_pct/100)) / div_apuesta
