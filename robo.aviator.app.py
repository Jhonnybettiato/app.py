...
st.set_page_config(page_title="Aviator Elite Robot v9.6.7", page_icon="🦅", layout="wide")
...
st.markdown("""
    ...
    /* Efecto de parpadeo en el borde del input para indicar FOCO */
    @keyframes blink-border {
        0% { border-color: #333; box-shadow: 0 0 0px #00ff41; }
        50% { border-color: #00ff41; box-shadow: 0 0 10px #00ff41; }
        100% { border-color: #333; box-shadow: 0 0 0px #00ff41; }
    }
    .stTextInput input {
...
# --- INTERFAZ ---
st.markdown('<div class="main-header">🦅 AVIATOR ELITE ROBOT v9.6.7</div>', unsafe_allow_html=True)
...
c2.markdown(f'<div class="{cl_res}"><p class="label-elite">GANANCIA</p><h2 class="valor-elite">{int(res_ac):,}</h2></div>', unsafe_allow_html=True)
...
for v in reversed(st.session_state.historial):
    if v >= 10.0: break
    sin_rosa += 1
col_sem = "#27ae60" if sin_rosa >= 30 else "#f39c12" if sin_rosa >= 25 else "#333"
st.markdown(f'<div class="semaforo" style="background-color:{col_sem}; padding:20px; border-radius:15px; margin:10px 0; text-align:center;"><h2 style="color:white; margin:0;">⏳ ESPERAR ({sin_rosa}/30)</h2></div>', unsafe_allow_html=True)

# PANEL DE REGISTRO (LA SOLUCIÓN AL FOCO)
st.markdown('<div class="elite-card">', unsafe_allow_html=True)
r1, r2, r3, r4 = st.columns([2, 1, 1, 1])

with r1:
    # EL TRUCO: Al usar st.empty(), forzamos a Streamlit a renderizar el componente de nuevo
    # con su nueva clave, lo que hace que el navegador le asigne el foco automáticamente.
